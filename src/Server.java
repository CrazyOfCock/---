...
public class Server {
    ...
    public Server() throws IOException {
        ...
        serverSocket = new ServerSocket(Protocol.PORT_NUM);
        System.out.println("Chat Server Listening on port " + Protocol.PORT_NUM);
    }
        ...
        public void run() {
            ...
            try {
                ...
                while ((line = is.readLine()) != null) {
                    ...
                    switch (messageType) {
                        case Protocol.CMD_LOGIN:  // 登录消息类型：A + login（登录名）
                            // 登录信息内不包含登录名
                            if (!Protocol.isValidLoginName(line)) {
                                // 回复登录消息，登录信息不合法
